...
function ProgressPageContent() {
  ...
  const handleFollowUpSent = (userMessage: string, assistantReply: string) => {
  ...
  const PROGRESS_PDF_LABELS = {
    es: {
      ...
      conversationTitle: "Conversación con EduGuIA",
      assessmentPhase: "Evaluación adaptativa",
      followUpPhase: "Seguimiento posterior",
      teacher: "Docente:",
      ai: "EduGuIA:",
      typeAssessment: "Evaluación",
      typeObservation: "Observación",
      typeMilestone: "Hito",
      typeIntervention: "Seguimiento IA",
      noConversation: "Sin conversación registrada.",
      noTimeline: "Sin entradas en la línea de tiempo.",
    ...
    en: {
      ...
      conversationTitle: "Conversation with EduGuIA",
      assessmentPhase: "Adaptive assessment",
      followUpPhase: "Follow-up",
      teacher: "Teacher:",
      ai: "EduGuIA:",
      typeAssessment: "Assessment",
      typeObservation: "Observation",
      typeMilestone: "Milestone",
      typeIntervention: "AI follow-up",
  ...
  const handleDownloadReport = async () => {
    ...
    try {
      ...
      const pdfLabels: ConversationPdfLabels = {
        sectionTitle: labels.conversationTitle,
        assessmentPhase: labels.assessmentPhase,
        followUpPhase: labels.followUpPhase,
        teacher: labels.teacher,
        ai: labels.ai,
        timeline: labels.timeline,
        typeAssessment: labels.typeAssessment,
        typeObservation: labels.typeObservation,
        typeMilestone: labels.typeMilestone,
        typeIntervention: labels.typeIntervention,
        noConversation: labels.noConversation,
        noTimeline: labels.noTimeline,
      };
      ...
      const conversationBlocks = buildConversationBlocks(savedSelected.assessmentData);
      renderConversationSection(pdfCtx, conversationBlocks, pdfLabels);
      cursorY = pdfCtx.getY() + 4;

      renderTimelineSection(pdfCtx, timeline, pdfLabels);
      cursorY = pdfCtx.getY() + 4;
