  ...
  return (
    <div className="p-6 lg:p-8">
      ...
      <header className="mb-8">
        ...
        <p className="mt-2 text-muted-foreground">{curso.descripcion || "Sin descripción"}</p>
      </header>

      <div className="grid gap-6 lg:grid-cols-2">
        <Card>
          <CardHeader>
            <CardTitle>Módulos / clases</CardTitle>
          </CardHeader>
          <CardContent className="space-y-4">
            <form onSubmit={(e) => void addModulo(e)} className="flex gap-2">
              <Input
                placeholder="Nombre del módulo o clase"
                value={moduloTitulo}
            ...
            <ul className="space-y-2">
              {modulos.map((mod) => (
                <li key={mod.id} className="rounded-lg border p-3">
                  <button
                    type="button"
                    className={`font-medium ${selectedModulo === mod.id ? "text-primary" : ""}`}
        ...
        <Card>
          <CardHeader>
            <CardTitle>Materiales y actividades</CardTitle>
          </CardHeader>
          <CardContent className="space-y-4">
            {modulos.length === 0 ? (
              <p className="text-sm text-muted-foreground">Primero crea un módulo.</p>
            ) : (
              <>
                <form onSubmit={(e) => void addMaterial(e)} className="space-y-3 rounded-lg border p-3">
                  <Label>Agregar material (video, PDF, enlace…)</Label>
                  <Input
                  ...
                  <Select value={materialTipo} onValueChange={(v) => setMaterialTipo(v as Material["tipo"])}>
                    ...
                    <SelectContent>
                      <SelectItem value="video">Video / clase grabada</SelectItem>
                  ...
                  <Input
                    placeholder="URL (YouTube, Drive, PDF…)"
                    value={materialUrl}
                  ...
                  <div className="space-y-3 text-sm">
                    <div>
                      ...
                      <ul className="mt-1 list-disc pl-5">
                        {(materiales[selectedModulo] ?? []).map((m) => (
                          <li key={m.id}>
                            {m.titulo} ({m.tipo})
                            {m.url && (
                              <>
                                {" — "}
                                <a href={m.url} target="_blank" rel="noopener" className="text-primary underline">
                                  ver
                                </a>
                              </>
                            )}
